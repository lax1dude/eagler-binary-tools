...
public class OptimizedOBJConverter {
	...
	public static void _main(String[] args, boolean v1_8) throws IOException {
		if (args.length != 3) {
			System.out
					.println("Usage: obj2mdl-" + (v1_8 ? "1.8" : "1.5") + " <input file> <output file> <texture mode>");
	...
	public static void convertModel(Collection<String> lines, OutputStream out, boolean textureMode, boolean v1_8)
			throws IOException {
		...
		for (String ul : lines) {
			...
			if (l[0].equals("vn")) {
				int dumb = v1_8 ? 0 : 127;
				normals.add(new byte[] { (byte) ((int) (Float.parseFloat(l[1]) * 127.0F) + dumb),
						(byte) ((int) (Float.parseFloat(l[2]) * 127.0F) + dumb),
						(byte) ((int) (Float.parseFloat(l[3]) * 127.0F) + dumb), (byte) 0 });
			}
			if (textureMode) {
				if (l[0].equals("vt")) {
			...
			if (l[0].equals("f")) {
				...
				String[] v2 = l[2].split("/");
				String[] v3 = l[3].split("/");
				faces.add(new int[][] { { Integer.parseInt(v1[0]), Integer.parseInt(v1[1]), Integer.parseInt(v1[2]) },
		...
		for (byte[] v : vboentries) {
			...
			// (IndexLinkedHashSet is backed by an arraylist for indices and a HashSet for
			// performance)
